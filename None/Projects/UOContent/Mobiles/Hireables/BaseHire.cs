...
namespace Server.Mobiles
{
    public class BaseHire : BaseCreature
    {
        ...
        public BaseHire()
            : base(AIType.AI_Melee, FightMode.Aggressor)
        {
            ControlSlots = 2;
        }
                ...
                if (!Controlled)
                {
                    ...
                    if (item is Gold)
                    {
                        ...
                        if (item.Amount >= Pay)
                        {
                            ...
                            if (AddHire(from))
                            {
                                int daysOfWork = item.Amount / Pay;
                                string message = $"I thank thee for paying me. I will work for thee for {daysOfWork} day{(daysOfWork > 1 ? "s" : "")}.";
                                SayTo(from, true, message);
        ...
        {
            Say(1043256, string.Format("{0}", Pay));// "I am available for hire for ~1_AMOUNT~ gold coins a day. If thou dost give me gold, I will work for thee."
        }
