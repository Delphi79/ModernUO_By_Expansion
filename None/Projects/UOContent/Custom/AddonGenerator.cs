	...
	public class {name}AddonDeed : BaseAddonDeed
	{
		...
		private static void OnAddonGen( CommandEventArgs e )
		{
            ...
            if (e.Arguments.Length > 0)
            {
                ...
                BoundingBoxPicker.Begin(e.Mobile, (map, start, end) => PickerCallbackWrapper(e.Mobile, map, start, end, PickerCallback, state));
        ...
        private static void PickerCallback( Mobile from, Map map, Point3D start, Point3D end, object state )
		{
            ...
            if (statics)
            {
                for (int x = start.X; x <= end.X; x++)
                {
                    for (int y = start.Y; y <= end.Y; y++)
                    {
                        ...
                        StaticTile[] staticTiles = tileMatrix.GetStaticTiles(x, y);
                        LandTile landTile = tileMatrix.GetLandTile(x, y);
			...
			if ( m_CustomOutputDirectory != null )
				path = Path.Combine( m_CustomOutputDirectory, string.Format( @"Addon Scripts\{0}Addon.cs", name ) );
			else
				path = Path.Combine( Core.BaseDirectory, string.Format( @"Addon Scripts\{0}Addon.cs", name ) );
			...
			if ( ! fail )
			{
                ...
                string script = string.Format("Script content:{0}{1}", Environment.NewLine, output);
                from.SendMessage(0x40, $"Script saved to {path}{Environment.NewLine}{script}");
		...
		private class InternalGump : Gump
		{
			...
			public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
			{
				...
				if ( Verify( m_State ) )
				{
                    BoundingBoxPicker.Begin(sender.Mobile, (map, start, end) => PickerCallbackWrapper(sender.Mobile, map, start, end, PickerCallback, m_State));
